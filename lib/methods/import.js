...
module.exports = function (options, instances) {
	...
	return async function () {
		...
		try {
			...
			if (!await fs.exists(options.import.logPath)) {
				await fs.writeFile(options.import.logPath);
			}
			...
			for (const file of fileNames) {
				...
				data.path = options.import.logPath;
				data.logName = file;
...
const execSteps = async function (step, list, row, index, callbacks, data, cbArgs) {
	if (step < callbacks.length) {
		try {
			const insertItem = await callbacks[step]({
			...
			if (insertItem && typeof insertItem === 'object') {
				return execSteps(step + 1, list, row, index, callbacks, data, insertItem || {});
			} else {
...
const logFile = function (index, logPath, name, fileName) {
	return async function (type, text) {
		...
		const appendText = `[${datetime}] [${type}] [Index: ${index}] [File: ${fileName} | Name: ${name}] | ${text}`;

		try {
			await fs.appendFile(path.join(logPath, name), appendText);
		} catch (err) {
